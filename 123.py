...
class run_new:
    ...
    buy = float(0.20000)  # 买入价格
    num = float(8.0000)  # 买入数量
    num_thread = 35  # 线程数量 运行次数
    ...
    data = {"is_margin": 0, "engine_type": "normal", "symbol": "watt_usdt", "fundpass": "", "captcha": "",
    ...
    def start(self):
        a = threading.Thread(target=self.queue_uee)
        a.start()
        threads = []
