...
class run_new():
    ...
    data = {"is_margin": 0, "engine_type": "normal", "symbol": "grv_usdt", "fundpass": "", "captcha": "",
            "type": "ask", "rate": buy, "vol": num, "order_type": "", "iceberg": 0, "source": "web"}

    def run_1(self):
        # while True:
            q = queue.Queue()
            response = requests.post("https://www.gate.io/json_svr/exchange/?u=1&c=782289", data=self.data, headers=self.headers)
            q.put(response)
            q.task_done()
            q.get()
            # muu = q.put()
    ...
    def start(self):
        ...
        for x in range(self.num_thread):
            t = threading.Thread(target=self.run_1)
            threads.append(t)
            t.start()
